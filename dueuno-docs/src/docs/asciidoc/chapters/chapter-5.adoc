...
Now we are going to finish our first Dueuno application.
...
. The class name starts with `T`. This is a Dueuno convention to let us immediately realize when we are working on a domain object. `T` stands for _Table (I know, it’s such an original decision)._
...
We don’t like writing the _Business Logic_ in controllers. It is an _anti-pattern_. Dueuno uses controllers as a way to implement the _User Interface_.
...
Now, with a bit of luck, we should be able to run our first complete Dueuno application:
...
This chapter closes the first round on the Dueuno basics.
