...
<g:else>
    <g:if test="${c.image}"><i><asset:image src="${c.image}" class="${c.text ? 'me-1' : ''} ${c.imageClass}" style="${c.imageStyle}"/></i></g:if>
    <g:elseif test="${c.icon}"><render:icon icon="${c.icon}" class="${c.text ? 'me-1' : ''} ${c.iconClass} loading-off" style="${c.iconStyle}"/></g:elseif>
